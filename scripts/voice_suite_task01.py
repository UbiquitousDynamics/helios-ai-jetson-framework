...
class EventTrace:
    ...
    def __init__(self, case_ids: set[str], *, maximum_records: int, clock=time.perf_counter_ns):
        ...
        require(
            bool(case_ids) and all(isinstance(value, str) for value in case_ids),
            "invalid_case_catalog",
        )
        self.run_id = uuid.uuid4()
        self._correlations = {key: str(uuid.uuid5(self.run_id, key)) for key in case_ids}
        self._fixtures = {
            key: f"task01-{index:03d}" for index, key in enumerate(sorted(case_ids), 1)
        }
    ...
    def record(
        ...
        case_id: str,
        event: E | R,
        snapshot: ConversationFloorSnapshot,
        *,
        outcome: str,
        ...
        require(
            response_id is None or (type(response_id) is int and response_id > 0),
        ...
        with self._lock:
            ...
            require(
                not self._rows or stamp >= self._rows[-1]["monotonic_ns"], "trace_clock_reversed"
            )
            self._rows.append(
                {
                    "schema_version": 1,
                    "task_id": "01",
                    "evidence_layer": "local_logic",
                    "run_id": str(self.run_id),
                    "correlation_id": self._correlations[case_id],
                    "fixture_id": self._fixtures[case_id],
                    "host_id": self._host_id,
                    "event_id": len(self._rows) + 1,
                    ...
                    "floor_state": snapshot.state.value,
                    "revision": snapshot.revision,
                    "candidate_return_state": (
                        snapshot.candidate_return_state.value
                        if snapshot.candidate_return_state
                        else None
                    ),
                }
            )
...
def matrix_cases(contract: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    require(
        ...
        and contract.get("test_task") == "01",
        "unsupported_transition_contract",
    )
    ...
    require(
        set(contract["event_kinds"]) == kinds and len(contract["event_kinds"]) == len(kinds),
    ...
    expected_contexts |= {
        (S.BARGE_IN_CANDIDATE.value, state.value)
    ...
    for context in contexts.values():
        ...
        require(
            type(context.get("expected_revision")) is int and context["expected_revision"] >= 0,
            "invalid_expected_revision",
        )
        require(
            isinstance(context.get("event_prefix"), list)
            and all(isinstance(kind, str) and kind in kinds for kind in context["event_prefix"]),
    ...
    require(
        observed_contexts == expected_contexts and len(contexts) == len(expected_contexts),
    ...
    for source, row in matrix.items():
        ...
        require(
            all(value is None or value in contexts for value in row.values()),
...
def validate_profile(profile: dict[str, Any], case_count: int) -> None:
    required = {
        "transition_verdict_errors",
        "transition_state_errors",
        "revision_errors",
        "rejected_event_mutations",
        "snapshot_identity_errors",
    }
    require(
        type(profile["required_matrix_cases"]) is int
        and profile["required_matrix_cases"] == case_count,
        "matrix_sample_count",
    )
    require(
        type(profile["maximum_trace_records"]) is int
        and profile["maximum_trace_records"] >= case_count,
        "insufficient_trace_bound",
    )
    ...
    for rule in invariants.values():
        require(
            ...
            and rule.get("unit") == "cases"
            and rule.get("mandatory") is True,
            "invalid_invariant_definition",
        )
        require(
            type(rule.get("maximum")) is int and rule["maximum"] == 0,
            "logical_invariant_cannot_be_relaxed",
        )
        require(
            type(rule.get("minimum_samples")) is int and rule["minimum_samples"] == case_count,
            "invariant_sample_count",
        )
...
def run_matrix(contract: dict[str, Any], profile: dict[str, Any]) -> tuple[dict, list, list]:
    ...
    for source, kind in cases:
        ...
        require(
            before.state.value == source_context["state"]
            and (before.candidate_return_state.value if before.candidate_return_state else None)
            == source_context["candidate_return_state"],
            "context_prefix_mismatch",
        )
        ...
        if expected_legal:
            ...
            return_state = (
                after.candidate_return_state.value if after.candidate_return_state else None
            )
            if (
                after.state.value != target["state"]
                or return_state != target["candidate_return_state"]
            ):
                violations.append("transition_state_errors")
            changed = (after.state, after.candidate_return_state) != (
                before.state,
                before.candidate_return_state,
            )
        ...
        results.append(
            {
                "case_id": case_id,
                "correlation_id": trace.correlation(case_id),
...
def hil_prerequisites(manifest: dict[str, Any], dependencies: dict[str, str | None]) -> dict:
    ...
    if valid_devices:
        for device in devices.values():
            if not isinstance(device, dict):
                ...
                break
            valid_devices &= (
                type(device.get("index")) is int
                and device["index"] >= 0
                and isinstance(device.get("host_api"), str)
                and bool(device["host_api"].strip())
                and isinstance(device.get("name"), str)
                and bool(device["name"].strip())
                and device["name"].strip().casefold() != "default"
                and type(device.get("sample_rate_hz")) is int
                and device["sample_rate_hz"] > 0
                and type(device.get("channels")) is int
    ...
    if not isinstance(isolation, dict) or not all(
        ...
        for key in (
            "generated_speech_only",
            "isolated_speakers_or_headphones",
            "monitoring_and_loopback_disabled",
            "no_people_in_capture_area",
        )
    ):
    ...
    if not isinstance(limits, dict) or not all(
        finite_number(limits.get(key)) and limits[key] > 0
        for key in (
    ...
    missing = [
        name
        for name in ("piper-tts", "vosk", "sounddevice", "PyAudio", "onnxruntime")
    ...
    return {
        ...
        "device_inventory_performed": False,
        "devices_opened": 0,
        "acoustic_exchanges": 0,
...
def run(output: Path, profile_name: str, hardware_path: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "local_matrix": summary,
        "hil": hil,
        "environment": env,
        "implementation": implementation_manifest(ROOT),
        "threshold_profile": profile_name,
        "threshold_version": profiles["profile_version"],
        "thresholds": profile["invariants"],
        "spec_sha256": {
            "task01-transitions.json": sha256(contract_path),
