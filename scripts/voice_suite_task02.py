...
_METRICS = frozenset(
    {"authority_mismatches", "final_count_mismatches", "provisional_sink_admissions"}
)
_SCENARIOS = frozenset(
    {
        "en_revised_final",
        "it_revised_final",
        "stale_capture_and_segment",
        "stale_revision_and_empty_final",
        "provisional_only_timeout",
        "en_identical_distinct_segments",
        "it_identical_distinct_captures",
        "capture_restart_resets_segment_revision",
        "stale_partial_after_new_capture",
...
def validate(contract: dict[str, Any], profile: dict[str, Any]) -> list[dict[str, Any]]:
    require(
        ...
        and contract.get("test_task") == "02",
        "invalid_contract_version",
    )
    scenarios = contract.get("scenarios")
    require(isinstance(scenarios, list) and len(scenarios) >= 10, "insufficient_scenarios")
    require(
        type(profile.get("minimum_scenarios")) is int
        and profile["minimum_scenarios"] == 10
        and len(scenarios) >= 10,
        "invalid_scenario_threshold",
    )
    require(
        type(profile.get("maximum_events")) is int and 0 < profile["maximum_events"] <= 1024,
        "invalid_event_bound",
    )
    require(
        isinstance(profile.get("invariants"), dict) and set(profile["invariants"]) == _METRICS,
        "invariant_coverage",
    )
    for rule in profile["invariants"].values():
        require(
            isinstance(rule, dict)
            and type(rule.get("maximum")) is int
            and rule["maximum"] == 0
            and type(rule.get("minimum_samples")) is int
            and rule["minimum_samples"] == 10,
    ...
    for scenario in scenarios:
        ...
        require(
            type(scenario.get("expected_final_count")) is int
            and 0 <= scenario["expected_final_count"] <= len(events),
            "invalid_final_count",
        )
        require(
            scenario["expected_final_count"]
            == sum(event.get("expected") == "final" for event in events if isinstance(event, dict)),
        ...
        for event in events:
            require(
                ...
                and type(event.get("is_final")) is bool
                and event.get("expected") in {"provisional", "final", "ignored"},
            ...
            for key in ("capture_id", "segment_id", "revision"):
                ...
                value = event.get(key)
                require(
                    value is None or (type(value) is int and value > 0), "invalid_segment_identity"
                )
            declared_events[event["expected"]] += 1
    require(ids == _SCENARIOS and declared_events == _EXPECTED_EVENTS, "contract_coverage_missing")
...
def run_local(contract: dict[str, Any], profile: dict[str, Any]) -> tuple[dict, list, list]:
    ...
    for case in scenarios:
        ...
        samples["final_count_mismatches"].append(int(final_count != case["expected_final_count"]))
        cases.append(
            {
                "fixture_id": case["id"],
                "correlation_id": correlation_id,
    ...
    local_status = (
        "failed" if "failed" in statuses else ("passed" if statuses == {"passed"} else "blocked")
    )
    return (
        {
            "status": local_status,
            ...
            "metric_summaries": metric_results,
        },
        cases,
        rows,
    )
...
def run(output: Path, profile_name: str, hardware_path: Path) -> dict[str, Any]:
    ...
    require(
        ...
        and profiles.get("profile_version") == "1.0.0"
        and profiles.get("test_task") == "02",
    ...
    reasons = []
    if any(
        hardware.get("devices", {}).get(key) is None
        for key in ("pc_input", "pc_output", "helios_input", "helios_output")
    ):
    ...
    if not all(
        hardware.get("isolation", {}).get(key) is True
        for key in (
    ...
    if any(
        env["dependencies"].get(name) is None
    ...
    report = {
        ...
        "spec_sha256": {
            "task02-contract.json": sha256(contract_path),
