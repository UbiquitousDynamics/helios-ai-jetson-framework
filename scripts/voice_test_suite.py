...
SPEC_FILES = ("fixtures.json", "metrics.json", "thresholds.json", "traceability.json",
              "hardware.example.json")
DEPENDENCIES = ("pytest", "numpy", "httpx", "ruff", "piper-tts", "vosk",
                "sounddevice", "PyAudio", "onnxruntime")
CATEGORIES = {"normal", "pause_correction", "barge_in", "control", "task_steering",
              "noise", "failure"}
RECIPE_KINDS = {"fake_backend_task", "injected_failure", "offline_control",
                "overlapping_piper_speech", "piper_segments", "piper_speech",
                "piper_with_noise", "policy_failure", "session_sequence", "silence",
                "target_tts_echo"}
REQUIRED_METRICS = {
    "end_to_end_first_audio_ms", "final_answer_ms", "stt_ms", "model_ttft_ms",
    "model_generation_ms", "tts_first_audio_ms", "tts_synthesis_ms", "interruption_ms",
    "endpointing_ms", "premature_finalization_rate", "wer", "cer", "exact_final_rate",
    "duplicate_dispatch_rate", "dropped_turn_rate", "partial_only_turn_rate",
    "false_barge_in_rate", "missed_barge_in_rate", "response_completeness_rate",
    "unexpected_overlap_rate", "task_transition_ms", "verified_completion_precision",
    "verified_completion_recall", "cpu_percent", "ram_mib", "temperature_c", "power_w",
    "queue_depth", "leaked_workers", "cleanup_ms", "unauthorized_egress_count",
}
REQUIRED_EVENTS = {
    "stimulus_synthesis_start", "stimulus_ready", "stimulus_playback_start",
    "stimulus_playback_end", "pc_microphone_capture_start", "pc_microphone_capture_end",
    "first_provisional_recognition", "authoritative_final_recognition", "model_dispatch",
    "model_start", "model_first_token", "model_completion", "tts_synthesis_start",
    "tts_synthesis_end", "helios_audio_playback_start", "pc_first_response_energy",
    "response_capture_end", "interruption_confirmed", "task_backend_acknowledgement",
}
TASKS = {f"{n:02d}" for n in range(17)}
IMPLEMENTATION = ("config.py", "assistant.py", "audio/tts.py", "audio/speech_pipeline.py",
                  "audio/backchannel.py", "recognizer/speech_recognizer.py",
                  "recognizer/turn_endpoint_detector.py", "recognizer/barge_in_detector.py",
                  "recognizer/echo_suppression_policy.py", "api/realtime_conversation.py",
                  "api/control_intents.py", "api/transcripts.py", "api/api_client.py",
                  "api/streaming.py", "api/metrics.py", "observability/resources.py")
...
def records(data: dict[str, Any], key: str) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        require(isinstance(identifier, str) and
                re.fullmatch(r"[A-Za-z0-9_-]+", identifier) is not None, "invalid_record_id")
...
def _validate_specs(bundle: dict[str, dict[str, Any]], root: Path) -> dict[str, int]:
    ...
    require(catalog["synthesis"]["entrypoint"] == "audio.tts.PiperTTS.synthesize_wave",
            "wrong_synthesis_path")
    require(catalog["catalog_version"] == "1.0.0" and thresholds["profile_version"] == "1.0.0",
    ...
    for voice in catalog["voices"].values():
        ...
        require(type(voice["sample_rate_hz"]) is int and voice["sample_rate_hz"] > 0 and
                voice["sample_rate_hz"] == config["audio"]["sample_rate"], "voice_sample_rate")
    covered = set()
    for row in fixtures.values():
        ...
        require(isinstance(row["response_constraints"], list) and
                bool(row["response_constraints"]) and
                all(isinstance(v, str) and bool(v) for v in row["response_constraints"]),
                "fixture_response_constraints")
        require(isinstance(row["metric_ids"], list) and bool(row["metric_ids"]) and
                set(row["metric_ids"]) <= metrics.keys(), "fixture_unknown_metric")
        require(isinstance(row["test_tasks"], list) and bool(row["test_tasks"]) and
                set(row["test_tasks"]) <= TASKS, "fixture_unknown_task")
        require(isinstance(row["recipe"], dict) and row["recipe"].get("kind") in RECIPE_KINDS,
                "fixture_recipe")
    require(covered == {(lang, category) for lang in ("it", "en") for category in CATEGORIES},
    ...
    for event in events.values():
        require(event["clock_domain"] in {"pc", "target"} and bool(event["applicability"]),
                "event_clock_domain")
    ...
    for profile in thresholds["profiles"].values():
        ...
        for key, limit in profile["metrics"].items():
            ...
            require(type(limit["minimum_samples"]) is int and limit["minimum_samples"] > 0,
                    "threshold_samples")
            require(limit["status"] in {"invariant", "provisional", "uncalibrated", "calibrated"},
                    "threshold_status")
            require(limit["statistic"] in {"p95", "maximum", "minimum", "rate", "count"},
                    "threshold_statistic")
            if limit["status"] == "uncalibrated":
                require(limit["value"] is None, "uncalibrated_threshold_value")
            else:
                require(finite_number(limit["value"]) and limit["value"] >= 0,
                        "invalid_threshold_value")
            if limit["status"] == "calibrated":
                require(bool(profile.get("calibration_artifact_sha256")) and
                        re.fullmatch(r"[0-9a-f]{64}", profile["calibration_artifact_sha256"])
                        is not None, "missing_calibration_evidence")
    ...
    require(source_digest == bundle["traceability.json"]["requirement_source_sha256"],
            "requirement_source_changed")
    for row in trace.values():
        require(isinstance(row["requirement"], str) and bool(row["requirement"]),
                "requirement_text")
        matching = [line for line in canonical_rows if line.startswith("| " + row["id"] + " |")]
        require(len(matching) == 1 and matching[0].split("|")[3].strip() == row["requirement"],
                "requirement_text_mismatch")
        require(isinstance(row["test_tasks"], list) and bool(row["test_tasks"]) and
                set(row["test_tasks"]) <= TASKS, "requirement_unknown_task")
        require(row["local_evidence"] == "not_run" and row["hil_evidence"] == "blocked",
                "baseline_cannot_verify_requirements")
    hardware = bundle["hardware.example.json"]
    require(set(hardware["devices"]) ==
            {"pc_input", "pc_output", "helios_input", "helios_output"}, "hardware_roles")
    require(all(value is None for value in hardware["devices"].values()),
            "example_must_not_select_devices")
    require(hardware["status"] == "unconfigured" and
            hardware["calibration"]["status"] == "unverified", "example_must_not_claim_calibration")
    require(hardware["artifacts"]["audio_retention"] in {"delete", "keep"}, "retention_policy")
    require(set(hardware["limits"]) == {"exchange_timeout_seconds", "session_timeout_seconds",
    ...
    return {"fixtures": len(fixtures), "metrics": len(metrics), "requirements": len(trace),
            "timestamp_events": len(events)}


def summarize(samples: list[float], limit: dict[str, Any], *,
              denominators: list[int] | None = None) -> dict[str, Any]:
    ...
    require(type(limit["minimum_samples"]) is int and limit["minimum_samples"] > 0,
            "threshold_samples")
    require(limit["status"] in {"calibrated", "invariant", "provisional", "uncalibrated"},
            "threshold_status")
    statistic = limit["statistic"]
    require(statistic in {"p95", "maximum", "minimum", "rate", "count"}, "threshold_statistic")
    value = limit["value"]
    require((value is None and limit["status"] == "uncalibrated") or
            (finite_number(value) and value >= 0 and limit["status"] != "uncalibrated"),
            "invalid_threshold_value")
    ...
    if statistic == "rate":
        if denominators is None:
            require(not samples or not str(limit.get("unit", "ratio")).startswith("edits/"),
                    "rate_denominators_required")
            denominators = [1] * len(samples)
        require(len(denominators) == len(samples) and
                all(type(v) is int and v > 0 and finite_number(v) for v in denominators),
    ...
    result = {"sample_count": len(samples), "median": None, "p95": None, "maximum": None,
              "minimum": None, "failure_count": failures, "status": "unverified"}
    if statistic == "rate":
    ...
    if ordered:
        ...
        result.update(median=statistics.median(ordered), maximum=ordered[-1], minimum=ordered[0],
                      p95=ordered[lower] + (ordered[upper] - ordered[lower]) * (position - lower))
        aggregate = (numerator / denominator if statistic == "rate" else
                     sum(ordered) if statistic == "count" else result[statistic])
        require(all(finite_number(result[key]) for key in ("median", "maximum", "minimum", "p95"))
                and finite_number(aggregate), "nonfinite_metric_aggregate")
        if statistic in {"rate", "count"}:
            result["value"] = aggregate
        # Even one observed hard breach is a failure, regardless of sample/calibration gaps.
        breached = value is not None and (aggregate > value if limit["direction"] == "max"
                                          else aggregate < value)
        if breached:
            result["status"] = "failed"
        elif len(samples) >= limit["minimum_samples"] and limit["status"] in {"calibrated", "invariant"}:
...
def environment() -> dict[str, Any]:
    ...
    return {"python": platform.python_version(), "system": platform.system(),
            ...
            "dependencies": versions, "device_versions": None,
            "clock": "time.monotonic_ns", "clock_resolution_seconds":
...
def implementation_manifest(root: Path) -> dict[str, Any]:
    try:
        ...
        head = None
    return {"commit": head, "identity": "commit_plus_file_hashes",
...
def artifact_root(path: Path, root: Path = ROOT) -> Path:
    ...
    if destination.is_relative_to(repository):
        ...
        require(allowed.resolve() == allowed and destination.is_relative_to(allowed),
...
def run_baseline(spec: Path, output: Path, profile: str, require_hil: bool = False) -> dict[str, Any]:
    ...
    for key, metric in metrics.items():
        rows.append({"correlation_id": correlation_id, "task_id": "00", "metric_id": key,
                     "profile_id": profile, "threshold_version": thresholds["profile_version"],
                     "eligible_count": 0, "missing_count": 0, "reason": "baseline_not_measured",
                     "unit": metric["unit"], "artifact_source": metric["artifact_source"],
                     "raw_samples": [], "threshold": thresholds["profiles"][profile]["metrics"][key],
                     **summarize([], thresholds["profiles"][profile]["metrics"][key])})
    voices = {}
    for language, voice in bundle["fixtures.json"]["voices"].items():
        voices[language] = {"model": voice["model"], "model_sha256": sha256(ROOT / voice["model"]),
                            "config_sha256": sha256(ROOT / voice["config"]),
                            "sample_rate_hz": voice["sample_rate_hz"],
                            "piper_version": env["dependencies"]["piper-tts"],
                            "audio_generated": False}
    blockers = ["explicit_pc_and_helios_device_ids_missing", "acoustic_calibration_missing",
                "capture_isolation_unconfirmed", "acoustic_runner_not_implemented_in_task00"]
    missing = [p for p in ("piper-tts", "vosk", "sounddevice", "PyAudio")
               if env["dependencies"][p] is None]
    if missing:
        blockers.append("pc_native_dependencies_missing")
    report = {"schema_version": 1, "task_id": "00", "correlation_id": correlation_id,
              "status": "blocked" if require_hil else "passed", "local_baseline": "passed",
              "hil": {"status": "blocked", "reasons": blockers, "missing_packages": missing,
                      "exchanges": 0, "devices_opened": 0},
              "counts": counts, "languages": ["en", "it"], "environment": env,
              "implementation": implementation_manifest(ROOT),
              "spec_sha256": {name: sha256(spec / name) for name in SPEC_FILES},
              "harness_sha256": sha256(Path(__file__)), "voices": voices,
              "threshold_profile": profile, "threshold_version": thresholds["profile_version"],
              "validation_duration_ms": (time.monotonic_ns() - started) / 1_000_000,
              "cleanup": {"status": "passed", "workers_started": 0, "streams_opened": 0,
                          "audio_files_created": 0, "transcripts_created": 0},
              "artifacts": ["results.json", "metrics.jsonl"], "next_task": "01"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-hil", action="store_true",
    ...
    try:
        ...
        print(json.dumps({"schema_version": 1, "task_id": "00", "status": "failed",
