...
from api.conversation_control import ConversationEvent, ConversationEventKind as E, ConversationFloor
...
def test_matrix_runner_reports_every_legal_and_illegal_pair_without_audio():
    ...
    assert all(case["correlation_id"] == event["correlation_id"] for case, event in zip(cases, events))
...
def test_late_lifecycle_is_bound_to_old_response_and_cannot_mutate_current(phase, old_termination):
    ...
    try:
        ...
        trace.record("current", E.PLAYBACK_STARTED, control.snapshot().floor,
                     response_id=current, outcome="accepted")
        rows = trace.rows()
...
def test_stage_failure_has_correlated_recovery_and_late_start_cannot_resurrect(stage):
    ...
    try:
        ...
        trace.record("exchange", E.PLAYBACK_STARTED, control.snapshot().floor,
                     response_id=response, outcome="accepted")
        control.response_event(response, stage)
        interrupted = control.snapshot()
        assert interrupted.floor.state.value == "interrupted"
        trace.record("exchange", stage, interrupted.floor,
                     response_id=response, outcome="accepted")
        control.response_event(response, R.PLAYBACK_STARTED)
        ...
        trace.record("exchange", E.RESPONSE_FINISHED, control.snapshot().floor,
                     response_id=response, outcome="accepted")
...
def test_capture_lease_snapshots_are_local_ownership_evidence_not_microphone_events():
    ...
    assert set(asdict(snapshot)) == {
        "floor", "capture_active", "response_id", "synthesizing", "generation_complete", "stopped",
    }
    control.stop()
...
def test_trace_is_content_free_bounded_and_detached_from_mutable_exports():
    ...
    trace.record("synthetic secret should not be exported", E.ACTIVATE,
                 floor.apply(ConversationEvent(E.ACTIVATE)), outcome="accepted")
    rows = trace.rows()
    ...
    with pytest.raises(task.SpecificationError, match="trace_capacity_exceeded"):
        trace.record("synthetic secret should not be exported", E.SILENCE,
                     floor.snapshot(), outcome="accepted")
...
def test_invalid_trace_fields_never_echo_input(field, value, error):
    ...
    arguments = dict(case_id="case", event=E.SILENCE, snapshot=ConversationFloor().snapshot(),
                     outcome="accepted", response_id=None)
...
def test_filled_manifest_cannot_impersonate_measured_calibration_or_acoustic_pass():
    ...
    for role in manifest["devices"]:
        manifest["devices"][role] = {"index": 0, "name": "synthetic fixture device",
                                      "host_api": "test", "sample_rate_hz": 22050, "channels": 1}
    manifest["isolation"] = {key: True for key in manifest["isolation"]}
    manifest["calibration"]["status"] = "verified"
    dependencies = {name: "fixture-version" for name in (
...
def test_cli_retains_local_success_and_separate_hil_blocked_result(tmp_path, capsys):
    code = task.main(["--artifact-dir", str(tmp_path), "--hardware-manifest",
    ...
    assert set(file.name for file in directory.iterdir()) == {"results.json", "cases.jsonl", "events.jsonl"}
