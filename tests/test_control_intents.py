...
@pytest.mark.parametrize(
    ...
    [
        ("it", "Emilia, basta!", I.STOP_SPEAKING),
        ("it", "silenzio", I.STOP_SPEAKING),
        ("en", "Emilia, stop speaking.", I.STOP_SPEAKING),
        ("en", "stop", I.STOP_SPEAKING),
        ("it", "disattiva audio", I.MUTE),
        ("en", "mute audio", I.MUTE),
        ("it", "riattiva audio", I.UNMUTE),
        ("en", "unmute", I.UNMUTE),
        ("it", "sospendi conversazione", I.SUSPEND_SESSION),
        ("en", "pause session", I.SUSPEND_SESSION),
        ("it", "riprendi sessione", I.RESUME_SESSION),
        ("en", "resume conversation", I.RESUME_SESSION),
        ("it", "termina conversazione", I.END_SESSION),
        ("en", "end session", I.END_SESSION),
        ("it", "annulla attività", I.CANCEL_TASK),
        ("en", "cancel the task", I.CANCEL_TASK),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "please explain stop signs",
        "do not stop",
        "say mute",
        "what does end session mean",
        "cancel the task tomorrow",
        "stop and tell me a story",
        "unstoppable",
        "stopwatch",
        "spiega la parola silenzio",
        "non disattiva audio",
        "hello world stop",
    ],
)
...
def test_control_parser_rejects_provisional_authority():
    with pytest.raises(TypeError):
        parse_control(
            ProvisionalRevision("stop", capture_id=1, segment_id=1, revision=1), language="en"
        )
...
def assistant_for(results=(), *, language="en", api=None, tts=None):
    return VoiceAssistant(
        ...
        speech_recognizer=FakeRecognizer(
            [RecognitionResult(text, is_final=True) for text in results]
        ),
...
@pytest.mark.parametrize(
    "entry", ["process_command", "process_rag_command", "_process_model_prompt"]
)
@pytest.mark.parametrize(
    "command",
    ["stop", "mute", "unmute", "pause session", "resume session", "end session", "cancel task"],
)
...
def test_end_session_requires_new_activation_and_resume_cannot_bypass_it():
    assistant = assistant_for(
        ["Emilia", "end session", "resume session", "a new question", "Emilia a new question"]
    )
    try:
...
def test_mute_persists_across_response_boundaries_and_unmute_restores_output():
    tts = FakeTTS()
    api = APIClient(
        client=FakeClient([chunk("A synthetic answer.", done=True)]), tts=tts, retry_wait=0
    )
...
def test_actual_rag_cancellation_still_prevents_late_speech_and_model_dispatch():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(
                assistant._process_rag_response, "synthetic query", Search(), cancellation=token
            )
            try:
