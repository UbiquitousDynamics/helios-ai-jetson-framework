...
def test_local_runner_passes_real_authority_boundary_with_per_metric_samples(specifications):
    ...
    assert {row["observed_authority"] for row in events} == {
        "provisional", "final", "ignored"
    }
    assert {row["correlation_id"] for row in events} == {
        case["correlation_id"] for case in cases
    }
    ...
    assert all(item["status"] == "passed" and item["maximum"] == 0
...
def test_insufficient_provisional_boundary_samples_cannot_pass(specifications):
    ...
    case = next(case for case in contract["scenarios"]
                if case["id"] == "provisional_only_timeout")
...
def test_contract_cannot_replace_final_coverage_with_provisional_only_cases(specifications):
    ...
    for case in contract["scenarios"]:
        ...
        case["events"] = [{"text": "Emilia synthetic unfinished request",
                           "is_final": False, "capture_id": 1, "segment_id": 1,
                           "revision": 1, "expected": "provisional"}]
...
def test_cli_emits_content_free_artifacts_and_never_promotes_hil(tmp_path, capsys):
    ...
    assert task.main(["--artifact-dir", str(tmp_path),
                      "--hardware-manifest", str(hardware)]) == 2
    ...
    assert {path.name for path in folder.iterdir()} == {
        "results.json", "cases.jsonl", "events.jsonl"
    }
    serialized = printed + "".join(path.read_text(encoding="utf-8") for path in folder.iterdir())
    assert all(event["text"] not in serialized
               for case in contract["scenarios"] for event in case["events"]
               if event["text"].strip())
    rows = [json.loads(line) for line in (folder / "events.jsonl").read_text(
...
def test_invalid_contract_cli_response_is_sanitized(tmp_path, monkeypatch, capsys):
    ...
    assert task.main(["--artifact-dir", str(output), "--hardware-manifest",
                      str(task.ROOT / "tests" / "voice_suite" / "hardware.example.json")]) == 1
    printed = capsys.readouterr().out
    assert json.loads(printed) == {"schema_version": 1, "task_id": "02",
                                   "status": "failed", "reason": "task02_validation_failed"}
